...
def init_v3_model(config, train_data, train_labels, device):
    network_args = config['network_args'].copy()
    top_k_channels = network_args.get('top_k_channels', 16)
    num_channels = network_args.get('num_channels', train_data.shape[1])
    num_samples = network_args.get('num_samples', train_data.shape[2])
    num_classes = network_args.get('num_classes', len(np.unique(train_labels)))
    ...
    importance = compute_channel_importance(train_data_tensor, train_labels_tensor)
    selected_channel_idx = select_motor_channels(importance, top_k=top_k_channels)
    selected_channel_idx = selected_channel_idx.detach().cpu().numpy()
    ...
    print(f"【V3初始化】选中通道数：{top_k_channels} | 索引：{selected_channel_idx}")
...
def train_cv(config, sub_id, datafile, epochs, fixed_split_seed=None):
    ...
    init_data = data[:int(0.2 * len(data))]
    init_labels = labels[:int(0.2 * len(data))]
    config['network_args'], selected_channel_idx = init_v3_model(config, init_data, init_labels, device)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        train_loader = DataLoader(eegDataset(train_data_fold, train_labels_fold), batch_size=config['batch_size'],
                                  shuffle=True, pin_memory=True, num_workers=0, worker_init_fn=worker_init)
        val_loader = DataLoader(eegDataset(val_data_fold, val_labels_fold), batch_size=config['batch_size'],
        ...
        params = [
            ...
            {'params': [p for n, p in net.named_parameters() if 'scale_logits' in n], 'lr': config['lr'] * 10,
             'weight_decay': 1e-3}
        ]
        optimizer = optim.Adam(params, lr=config['lr'], weight_decay=float(config['weight_decay']))
        ...
        for epoch in range(epochs):
            ...
            print(
                f"【{sub_id} | Epoch:{epochs}】折{fold + 1} | 轮{epoch + 1:3d}/{epochs:3d} | LR：{lr:.6f} | 训练损失：{train_loss / total_train:.4f} | 训练ACC：{train_acc:.4f} | 验证ACC：{val_acc:.4f}")
        ...
        if best_val_acc > global_best_acc:
            ...
            print(f"【{sub_id} | Epoch:{epochs}】全局最佳模型更新！")
    ...
    print(
        f"\n【{sub_id} | Epoch:{epochs}】CV汇总 | 平均ACC：{cv_mean_acc:.4f}±{cv_std_acc:.4f} | 全局最佳ACC：{global_best_acc:.4f} | 平均最佳轮数：{avg_best_epoch}")

    cv_summary = {
        'sub_id': sub_id, 'split_seed': split_seed, 'global_best_acc': global_best_acc,
        'cv_mean_acc': float(cv_mean_acc), 'cv_std_acc': float(cv_std_acc),
        'fold_best_acc': fold_best_acc, 'fold_best_epoch': fold_best_epoch,
        'avg_best_epoch': avg_best_epoch, 'weight_save_dir': weight_save_dir,
        'selected_channel_idx': selected_channel_idx, 'train_epochs': epochs
    }
...
def train_final(config, sub_id, datafile, avg_best_epoch, epochs, selected_channel_idx):
    ...
    num_classes = network_args.get('num_classes', len(np.unique(full_labels)))
    ...
    print(f"【Final初始化】复用CV通道 | 索引：{selected_channel_idx} | 数量：{len(selected_channel_idx)}")
    ...
    for epoch in range(avg_best_epoch):
        ...
        print(
            f"【{sub_id} | Epoch:{epochs}】最终模型 | 轮{epoch + 1:3d}/{avg_best_epoch:3d} | LR：{lr:.6f} | 训练损失：{train_loss_avg:.4f} | 训练ACC：{train_acc:.4f}")
    ...
    final_config.update({
        'epochs': avg_best_epoch, 'sub_id': sub_id, 'datafile': datafile,
        'scale_weights_path': os.path.join(out_path, 'final_scale_weights.npy'),
        'selected_channel_idx': selected_channel_idx, 'cv_train_epochs': epochs,
        'final_train_acc': final_train_acc,
        'final_train_loss': final_train_loss
    })
...
if __name__ == '__main__':
    ...
    CONFIG_PATH = r'D:/EEG_HandWriting_Project_Code/FineSTAN/config/handwriteformer.yaml'
    EPOCHS_LIST = list(range(100, 800, 100))

    if not os.path.exists(CONFIG_PATH):
        raise FileNotFoundError(f"配置文件不存在：{CONFIG_PATH}")

    subject_list = scan_subjects_data(DATA_ROOT)
    completed_subs = []  #  "sub19"

    for sub_id, train_datafile in subject_list:
        if sub_id in completed_subs:
            print(f"【{sub_id}】已完成所有epoch，跳过")
            continue

        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            print(f"开始训练：【{sub_id}】 | Epoch数：{train_epochs}")
            print(f"=====================================")
            ...
            # 修正默认网络名
            config.setdefault('network', 'FineSTAN')
            ...
            try:
                cv_out_path, avg_best_epoch, selected_channel_idx = train_cv(config, sub_id, train_datafile,
                                                                             train_epochs,
                                                                             fixed_split_seed=config['random_seed'])
