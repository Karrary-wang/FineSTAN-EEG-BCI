...
class FineImagNet_wo_DSGM(nn.Module):
    def __init__(self, num_classes=10, num_samples=1000, num_channels=32, embed_dim=32, pool_size=180,
        ...
        self.temporal_attn = SegmentedTemporalAttention(
            embed_dim, window_size, stride, num_samples, attn_norm_type, 0.2,
            temp_coeff_init, global_weight_init, seg_fusion_init
        )
    ...
    def _calc_cls_dim(self):
        with torch.no_grad():
            ...
            dummy = torch.randn(1, self.num_channels, self.num_samples, device=current_device)
            x = self._extract_features(dummy)
            B, D, T = x.shape
            pad_len = (self.pool_size - (T % self.pool_size)) % self.pool_size
            x_padded = F.pad(x, (0, pad_len), mode='constant', value=0) if pad_len > 0 else x
            x_pool = self.pool(x_padded)
            x_pool = rearrange(x_pool, 'b d n -> b n d')
...
if __name__ == "__main__":
    ...
    dummy_data = torch.randn(10, 32, 1000).to(device)
    model = FineImagNet_wo_DSGM(selected_channel_idx=list(range(16))).to(device)
