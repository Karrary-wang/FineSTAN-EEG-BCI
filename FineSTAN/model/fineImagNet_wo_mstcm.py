...
# ====================== 消融模型2：w/o MSTCM 最终版 ======================
class FineImagNet_wo_MSTCM(nn.Module):
    def __init__(self, num_classes=10, num_samples=1000, num_channels=32, embed_dim=32, pool_size=180,
        ...
        # ========== 消融：移除 MSTCM 多尺度卷积，替换为单尺度输入映射 ==========
        self.input_conv = nn.Conv2d(1, embed_dim, (1, 1), padding=(0, 0))  # 单尺度1x1卷积，保持维度
        ...
        # 空间卷积（完全保留，适配selected_channel_num）
    ...
    def _pre_attention_features(self, x):
        # ========== 消融：替换 MSTCM 为单尺度输入映射 ==========
        x = x.unsqueeze(1)  # (B, 1, C, T)
        x = self.input_conv(x)  # (B, embed_dim, C, T)
        x = self.input_bn(x)
        x = self.input_act(x)
        return x
    ...
    def _extract_features(self, x):
        ...
        x = self.motor_attention(x)  # 保留 ACSM
        x = self.spatial_act(self.spatial_bn(self.spatial_conv(x)))
        x = x.squeeze(dim=2)
        x = x + self.temporal_attn(x)  # 保留 SegSTA
        return x
...
if __name__ == "__main__":
    ...
    # 模拟通道索引（和完整模型一致）
    selected_idx = list(range(16))  # 假设选16个通道

    model = FineImagNet_wo_MSTCM(
        num_classes=5,
        num_samples=1000,
