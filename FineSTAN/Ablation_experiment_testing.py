...
def get_model_class(network_name):

    if network_name == 'FineImagNet_wo_ACSM':
        from model.fineImagNet_wo_acsm import FineImagNet_wo_ACSM
        return FineImagNet_wo_ACSM
    elif network_name == 'FineImagNet_wo_MSTCM':
        from model.fineImagNet_wo_mstcm import FineImagNet_wo_MSTCM
        return FineImagNet_wo_MSTCM
    elif network_name == 'FineImagNet_wo_SegSTA':
        from model.fineImagNet_wo_segsta import FineImagNet_wo_SegSTA
        return FineImagNet_wo_SegSTA
    elif network_name == 'FineImagNet_wo_DSGM':
        from model.fineImagNet_wo_dsgm import FineImagNet_wo_DSGM
        return FineImagNet_wo_DSGM
    elif network_name == 'FineImagNet_Baseline':
        from model.fineImagNet_baseline import FineImagNet_Baseline
        return FineImagNet_Baseline
    # 主模型：统一新名称 FineSTAN
    elif network_name == 'FineSTAN':
        from model.finestan import FineSTAN
        return FineSTAN
    else:
        raise ValueError(f"不支持的模型类型: {network_name}")
...
def init_model_args(train_config, test_data, test_labels):
    ...
    selected_channel_idx = train_config.get('selected_channel_idx', None)

    if selected_channel_idx is None or not isinstance(selected_channel_idx, (list, np.ndarray)):
...
def batch_test_all_subjects(config, data_root, output_root, network_name):
    ...
    network_dir = Path(output_root).resolve()
    print(f"\n=== 开始批量测试 | 模型：{network_name} | 路径：{network_dir} ===")
    ...
    for subject_dir in subject_dirs:
        ...
        for epoch_num, epoch_data in epoch_info.items():
            ...
            try:
                ...
                cv_model = ModelCls(**cv_args).to(device)
                ...
                final_model = ModelCls(**final_args).to(device)
    ...
    print("\n" + "="*70)
    print("【全部被试 - 最佳Final模型测试准确率汇总】")
    print("="*70)
    for sub_name, acc in subject_acc_list:
        print(f"{sub_name:<8} : {acc:.4f}")
    print("="*70)
    ...
    if all_subjects_summary:
        ...
        print(f"全局最优准确率：{global_summary['max_best_final_acc']:.4f} | 全局最低准确率：{global_summary['min_best_final_acc']:.4f}")
    print("=====================================")
...
if __name__ == '__main__':
    ...
    CONFIG_FILE = r'D:/EEG_HandWriting_Project_Code/FineSTAN/config/HI_Ablation_experiment.yaml'

    # ========== 模型&输出路径切换（消融实验核心配置，二选一使用） ==========
    # 1. 主模型 FineSTAN（完整模型）
    OUTPUT_ROOT = r'D:/EEG_HandWriting_Project_Code/FineSTAN/output/bci_handwriting_data_English/xxx'
    NETWORK_NAME = 'FineSTAN'

    # 2. 消融模型（按需启用，注释上面两行）
    # OUTPUT_ROOT = r'D:/EEG_HandWriting_Project_Code/SPD-HandWriteFormer/output/bci_handwriting_data_English/Ablation_Baseline'
    # NETWORK_NAME = 'FineImagNet_Baseline'

    # OUTPUT_ROOT = r'xxx/Ablation_wo_ACSM'
    # NETWORK_NAME = 'FineImagNet_wo_ACSM'

    # OUTPUT_ROOT = r'xxx/Ablation_wo_MSTCM'
    # NETWORK_NAME = 'FineImagNet_wo_MSTCM'

    # OUTPUT_ROOT = r'xxx/Ablation_wo_SegSTA'
    # NETWORK_NAME = 'FineImagNet_wo_SegSTA'

    # OUTPUT_ROOT = r'xxx/Ablation_wo_DSGM'
    # NETWORK_NAME = 'FineImagNet_wo_DSGM'
    # =================================================================
