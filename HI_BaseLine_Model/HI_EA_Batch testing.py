...
import time
import glob
import random  # 提前导入，避免setRandom报错
...
# 1. 替换模型导入：EEGConformer → EEGEA（匹配你的模型文件名EEG_EA.py）
...
def load_model_weights(model, weight_path, device):
    """
    安全加载EEGEA模型权重（移除Conformer的动态预热逻辑）
    """
    # 加载权重文件
    state_dict = torch.load(weight_path, map_location=device)

    # 【关键修改】EEGEA无需预热（初始化时已完成特征维度计算）
    # 直接加载权重（过滤不匹配的键）
    ...
    print(f"✅ 成功加载EEGEA权重：{weight_path} | 匹配参数数：{len(filtered_state_dict)}")
    return model


def scan_subjects_models(out_root, network_name='EEGEA'):
    """
    扫描所有被试的EEGEA训练模型路径（仅修改默认network_name）
    """
...
def test_subject(config, sub_id, datafile, epoch_num, model_path):
    ...
    with torch.no_grad():
        for batch_data, batch_labels in test_loader:
            ...
            # 3. EEGEA评估模式返回(特征, 输出)（和Conformer逻辑一致，无需修改）
...
if __name__ == '__main__':
    ...
    # 6. EEGEA模型参数（和训练时保持一致）
    config['network_args'].setdefault('chans', 32)
    config['network_args'].setdefault('samples', 1000)
    config['network_args'].setdefault('multiple', 9)
    config['network_args'].setdefault('num_classes', 10)
    config['network_args'].setdefault('dataset', 'handwriting')
    config['network_args'].setdefault('pool_kernels', [50, 100, 200])
