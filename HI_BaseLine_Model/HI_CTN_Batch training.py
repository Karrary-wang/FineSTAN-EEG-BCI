...
from model.ctnet import CTNet  # 导入CTNet模型
...
def train_cv(config, sub_id, datafile, epochs):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        # 初始化CTNet模型
        net = CTNet(**config['network_args']).to(device)

        # 模型预热：验证输入输出维度
        net.eval()
        with torch.no_grad():
            dummy_input = torch.randn(1, 1, 32, 1000).to(device)
            _, _ = net(dummy_input)  # CTNet始终返回(特征, 输出)
        net.train()
...
if __name__ == '__main__':
    ...
    CONFIG_PATH = r'F:/EEG-TransNet-main/config/Hand_Writing_CTN.yaml'
    ...
    config.setdefault('network', 'CTNet')
    config.setdefault('device', 'cuda:0')
    config.setdefault('k_folds', 5)
    config.setdefault('network_args', {})

    # CTNet核心参数（适配32通道手写想象数据）
    config['network_args'].setdefault('n_classes', 10)
    config['network_args'].setdefault('number_channel', 32)
    config['network_args'].setdefault('heads', 2)
    config['network_args'].setdefault('emb_size', 16)
    config['network_args'].setdefault('depth', 6)
    config['network_args'].setdefault('eeg1_f1', 8)
    config['network_args'].setdefault('eeg1_kernel_size', 64)
    config['network_args'].setdefault('eeg1_D', 2)
    config['network_args'].setdefault('eeg1_pooling_size1', 8)
    config['network_args'].setdefault('eeg1_pooling_size2', 8)
    config['network_args'].setdefault('eeg1_dropout_rate', 0.25)
    config['network_args'].setdefault('flatten_eeg1', 240)

    config['data_path'] = DATA_ROOT

    subject_list = scan_subjects_data(DATA_ROOT)
    completed_subs = ["sub01", "sub03", "sub05", "sub06", "sub07", "sub08", "sub10", "sub11", "sub12",
                      "sub13", "sub14", "sub15", "sub17", "sub18", "sub19", "sub20", "sub21", "sub22",
                      "sub23"]
    ...
    for sub_id, train_datafile in subject_list:
        ...
        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            print(f"开始训练 CTNet ：【{sub_id}】 | Epoch数：{train_epochs}")
            print(f"=====================================")
