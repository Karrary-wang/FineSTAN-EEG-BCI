...
def train_cv(config, sub_id, datafile, epochs):
    ...
    out_path = os.path.normpath(
        os.path.join(out_folder, 'ATCNet', sub_id, f'epoch_{epochs}', f'cv_{timestamp}'))
    os.makedirs(out_path, exist_ok=True)
    dictToYaml(os.path.join(out_path, 'cv_config.yaml'), config)

    device = torch.device(config.get('device', 'cuda:0') if torch.cuda.is_available() else 'cpu')
    print(f"【{sub_id} | Epoch:{epochs}】设备：{device}")
    ...
    print(f"【{sub_id}】CV路径：{out_path} | 数据形状：{data.shape}")

    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        print(f"\n【{sub_id}】第 {fold + 1}/{k_folds} 折训练启动")
        ...
        # 初始化模型（已完美适配原始 ATCNet）
        net = ATCNet(**config['network_args']).to(device)
        ...
        loss_func = nn.CrossEntropyLoss(label_smoothing=config.get('label_smoothing', 0.01))
        ...
        for epoch in range(epochs):
            ...
            print(f"【{sub_id}】折{fold+1} | 轮{epoch+1:3d}/{epochs} | 训练Loss:{train_loss/total_train:.4f} | 训练ACC:{train_acc:.4f} | 验证ACC:{val_acc:.4f}")
        ...
        print(f"【{sub_id}】第{fold + 1}折最佳ACC：{best_val_acc:.4f}")
...
def train_final(config, sub_id, datafile, avg_best_epoch, epochs):
    ...
    out_path = os.path.normpath(os.path.join(out_folder, 'ATCNet', sub_id, f'epoch_{epochs}', f'final_{timestamp}'))
    os.makedirs(out_path, exist_ok=True)
    ...
    print(f"\n【{sub_id}】开始最终训练（共{avg_best_epoch}轮）")
    for epoch in range(avg_best_epoch):
        ...
        train_acc = train_correct / total_train
        print(f"【{sub_id}】最终训练 | 轮{epoch+1:3d}/{avg_best_epoch} | Loss:{train_loss/total_train:.4f} | ACC:{train_acc:.4f}")

    final_model_path = os.path.join(out_path, 'final_model.pth')
    torch.save(current_net.state_dict(), final_model_path)
    print(f"【{sub_id}】最终模型已保存：{final_model_path}")
    return out_path
...
if __name__ == '__main__':
    ...
    CONFIG_PATH = r'F:/EEG-TransNet-main/config/Hand_Writing_ATCNet.yaml'
    ...
    completed_subs = []  # "sub01", "sub02", "sub03", "sub04", "sub06"
    ...
    for sub_id, train_datafile in subject_list:
        ...
        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            print(f"  开始训练 ATCNet：{sub_id} | Epoch={train_epochs}")
            print(f"=====================================")
            ...
            try:
                ...
                print(f"\n【{sub_id} | Epoch{train_epochs}】训练全部完成！")
