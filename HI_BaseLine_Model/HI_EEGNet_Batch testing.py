...
from model.EEGNet import EEGNetModel  # 导入EEGNet模型
...
def load_model_weights(model, weight_path, device):
    ...
    with torch.no_grad():
        ...
        model(dummy_input)  # EEGNet仅返回单个输出，不解包
    ...
    print(f" 成功加载EEGNet权重：{weight_path} | 匹配参数数：{len(filtered_state_dict)}")
    return model


def scan_subjects_models(out_root, network_name='EEGNetModel'):
    """
    扫描所有被试的EEGNet训练模型路径
    """
