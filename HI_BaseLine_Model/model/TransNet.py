...
# -------------------------- 核心修改：添加 classify_drop 参数 --------------------------
class TransNet(nn.Module):
    # 1. 在 __init__ 参数列表中添加 classify_drop，设置默认值 0.5（兼容旧代码）
    def __init__(self, num_classes=5, num_samples=1000, num_channels=32, embed_dim=32, pool_size=50,
                 pool_stride=15, num_heads=8, fc_ratio=4, depth=4, attn_drop=0.5, fc_drop=0.5, classify_drop=0.5):
        super().__init__()
        ...
        # 2. 用 classify_drop 初始化分类前的 Dropout 层
        self.dropout_classify = nn.Dropout(classify_drop)
        self.classify = nn.Linear(embed_dim * temp_embedding_dim, num_classes)

    def forward(self, x):
        # 核心修复：删除多余的unsqueeze操作（训练代码已处理维度）
        # x = x.unsqueeze(dim=1)  # 这行导致维度从4D变成5D，已删除
        ...
        # 3. 前向传播中添加分类前的 Dropout
        x = self.dropout_classify(x)
        out = self.classify(x)
