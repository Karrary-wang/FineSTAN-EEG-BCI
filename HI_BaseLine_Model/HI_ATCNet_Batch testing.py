...
try:
    ...
    print("错误: 无法导入 ATCNet 模型。请检查 model/ATCNet.py 是否存在。")
    sys.exit(1)
...
def scan_subjects_models(out_root, network_name='ATCNet'):
...
if __name__ == '__main__':
    ...
    args.setdefault('n_classes', 10)
    args.setdefault('in_chans', 32)
    args.setdefault('F1', 16)
    args.setdefault('D', 2)
    args.setdefault('kernLength', 64)
    args.setdefault('poolSize1', 8)
    args.setdefault('poolSize2', 7)
    args.setdefault('num_heads', 2)
    args.setdefault('depth', 2)
    args.setdefault('n_windows', 5)
