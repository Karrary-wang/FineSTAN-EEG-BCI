...
from model.TransNet import TransNet  # 替换为TransNet模型
...
def train_cv(config, sub_id, datafile, epochs, fixed_split_seed=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        # 初始化TransNet（从配置文件读取参数）
        net = TransNet(**config['network_args']).to(device)

        # 模型预热：初始化网络参数（TransNet无动态分类头，仅验证前向传播）
        net.eval()
        with torch.no_grad():
            dummy_input = torch.randn(1, 1, 32, 1000).to(device)
            _ = net(dummy_input)  # TransNet仅返回分类输出
        net.train()
    ...
    cv_summary = {'sub_id': sub_id, 'split_seed': split_seed, 'global_best_acc': global_best_acc,
...
if __name__ == '__main__':
    ...
    CONFIG_PATH = r'F:/EEG-TransNet-main/config/Hand_Writing_transnet.yaml'  # 改为TransNet配置文件
    EPOCHS_LIST = list(range(100, 400, 100))  # [100,200,300,400,500,600]
    ...
    config.setdefault('network', 'TransNet')  # 模型名称改为TransNet
    config.setdefault('device', 'cuda:0')
    config.setdefault('network_args', {})

    # TransNet核心默认参数（配置文件未定义时生效，优先读取YAML配置）
    config['network_args'].setdefault('num_classes', 10)
    config['network_args'].setdefault('num_samples', 1000)
    config['network_args'].setdefault('num_channels', 32)
    config['network_args'].setdefault('embed_dim', 32)
    config['network_args'].setdefault('pool_size', 50)
    config['network_args'].setdefault('pool_stride', 15)
    config['network_args'].setdefault('num_heads', 8)
    config['network_args'].setdefault('fc_ratio', 4)
    config['network_args'].setdefault('depth', 4)
    config['network_args'].setdefault('attn_drop', 0.5)
    config['network_args'].setdefault('fc_drop', 0.5)
    config['network_args'].setdefault('classify_drop', 0.5)

    config['data_path'] = DATA_ROOT

    subject_list = scan_subjects_data(DATA_ROOT)
    completed_subs = ["sub01", "sub03", "sub05", "sub06", "sub07", "sub08", "sub10", "sub11", "sub12", "sub13", "sub14"]  # 可自定义跳过已完成的被试
    # "sub01", "sub03", "sub05", "sub06", "sub07", "sub08", "sub10", "sub11", "sub12", "sub13", "sub14", "sub15", "sub17", "sub18", "sub19", "sub20", "sub21", "sub22", "sub23", "sub24"
    ...
    for sub_id, train_datafile in subject_list:
        ...
        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            print(f"开始训练 TransNet ：【{sub_id}】 | Epoch数：{train_epochs}")
            print(f"=====================================")
