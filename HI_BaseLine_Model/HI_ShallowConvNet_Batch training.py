...
def train_cv(config, sub_id, datafile, epochs):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        # 初始化 ShallowConvNet
        net = ShallowConvNet(**config['network_args']).to(device)
...
if __name__ == '__main__':
    ...
    CONFIG_PATH = r'F:/EEG-TransNet-main/config/Hand_Writing_ShallowConvNet.yaml'
    ...
    config['network_args'].setdefault('batch_norm', True)
    config['network_args'].setdefault('batch_norm_alpha', 0.1)
    ...
    for sub_id, train_datafile in subject_list:
        ...
        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            print(f"开始训练 ShallowConvNet ：【{sub_id}】 | Epoch数：{train_epochs}")
            print(f"=====================================")
