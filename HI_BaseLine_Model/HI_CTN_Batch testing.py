...
def load_model_weights(model, weight_path, device):
    ...
    with torch.no_grad():
        ...
        _, _ = model(dummy_input)  # CTNet始终返回(特征, 输出)
    ...
    print(f" 成功加载CTNet权重：{weight_path} | 匹配参数数：{len(filtered_state_dict)}")
    return model


def scan_subjects_models(out_root, network_name='CTNet'):
    """
    扫描所有被试的CTNet训练模型路径
    """
