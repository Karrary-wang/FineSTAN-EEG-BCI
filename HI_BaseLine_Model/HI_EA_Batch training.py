...
# 1. 替换模型导入：EEGConformer → EEG_EA（你的文件名是EEG_EA.py）
from model.EEG_EA import EEGEA
...
def train_cv(config, sub_id, datafile, epochs, fixed_split_seed=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        # 2. 替换模型初始化：EEGConformer → EEGEA
        net = EEGEA(**config['network_args']).to(device)

        # 【关键删除】EEGEA无需动态预热（已在初始化时预计算特征维度）
        # 移除EEGConformer的动态分类头预热代码
        # net.eval()
        # with torch.no_grad():
        ...
        for epoch in range(epochs):
            ...
            for batch_data, batch_labels in train_loader:
                ...
                optimizer.zero_grad()
                # EEGEA训练模式仅返回logits（和Conformer逻辑一致）
            ...
            with torch.no_grad():
                for batch_data, batch_labels in val_loader:
                    batch_data = batch_data.to(device, dtype=torch.float32)
                    batch_labels = batch_labels.to(device, dtype=torch.long)
                    # EEGEA评估模式返回(特征, logits)（和Conformer逻辑一致）
                    features, outputs = net(batch_data)
                    loss = loss_func(outputs, batch_labels)
...
if __name__ == '__main__':
    ...
    CONFIG_PATH = r'F:/EEG-TransNet-main/config/Hand_Writing_EA.yaml'
    EPOCHS_LIST = list(range(100, 700, 100))  # [100,200,300,400,500,600]
    ...
    # 3. 替换网络名称：EEGConformer → EEGEA
    config.setdefault('network', 'EEGEA')
    config.setdefault('device', 'cuda:0')
    config.setdefault('network_args', {})

    # EEGEA核心参数（适配32通道+1000采样点+10分类）
    config['network_args'].setdefault('chans', 32)          # 32通道
    config['network_args'].setdefault('samples', 1000)      # 1000采样点
    config['network_args'].setdefault('multiple', 9)        # 模型固定参数
    config['network_args'].setdefault('num_classes', 10)    # 10分类
    config['network_args'].setdefault('dataset', 'handwriting')  # 手写想象数据集
    config['network_args'].setdefault('pool_kernels', [50, 100, 200])  # 多尺度池化核

    config['data_path'] = DATA_ROOT

    subject_list = scan_subjects_data(DATA_ROOT)
    completed_subs = ["sub01", "sub03", "sub05", "sub06", "sub07", "sub08", "sub10", "sub11", "sub12", "sub13", "sub14", "sub15", "sub17", "sub18", "sub19", "sub20", "sub21", "sub22"]  # 可自定义跳过已完成的被试
    ...
    for sub_id, train_datafile in subject_list:
        ...
        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            # 打印改为EEGEA
            print(f"开始训练 EEGEA ：【{sub_id}】 | Epoch数：{train_epochs}")
            print(f"=====================================")
