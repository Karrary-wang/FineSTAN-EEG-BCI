...
if __name__ == '__main__':
    ...
    if all_test_results:
        ...
        # ===================== 【新增：行归一化平均混淆矩阵 + 保存 .npy】 =====================
        n_classes = config['network_args']['n_classes']
        sum_cm = np.zeros((n_classes, n_classes), dtype=np.float32)

        for sub_id, res in sub_best.items():
            cm = confusion_matrix(res['labels'], res['preds'], labels=np.arange(n_classes))
            row_sum = cm.sum(axis=1, keepdims=True) + 1e-8
            cm_norm = cm / row_sum
            sum_cm += cm_norm.astype(np.float32)

        avg_cm = sum_cm / len(sub_best)
        cm_save_path = os.path.join(OUT_ROOT, NETWORK_NAME, f"average_normalized_cm_{NETWORK_NAME}_{timestamp}.npy")
        np.save(cm_save_path, avg_cm)

        print(f"\n=====================================")
        print(f"【行归一化 0~1】所有被试最佳模型平均混淆矩阵")
        print(f"矩阵形状：{avg_cm.shape} | 保存路径：{cm_save_path}")
        print(f"=====================================\n")
        print(np.round(avg_cm, 4))
