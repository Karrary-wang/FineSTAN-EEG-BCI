...
from model.EEGConformer import EEGConformer  # 从model目录导入
...
def train_cv(config, sub_id, datafile, epochs, fixed_split_seed=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        # 模型预热：初始化动态分类头（解决首次前向维度适配）
        net.eval()
        with torch.no_grad():
            dummy_input = torch.randn(1, 1, 32, 1000).to(device)
            _ = net(dummy_input)  # 触发分类头初始化
        net.train()
        ...
        for epoch in range(epochs):
            ...
            for batch_data, batch_labels in train_loader:
                ...
                optimizer.zero_grad()
                # ========== 核心修复1：训练模式仅接收单个输出 ==========
                outputs = net(batch_data)  # 不再解包，直接接收分类输出
            ...
            with torch.no_grad():
                for batch_data, batch_labels in val_loader:
                    ...
                    batch_labels = batch_labels.to(device, dtype=torch.long)
                    # ========== 核心修复2：评估模式接收双返回值 ==========
                    features, outputs = net(batch_data)  # 评估模式返回(特征, 输出)
                    loss = loss_func(outputs, batch_labels)
...
if __name__ == '__main__':
    ...
    EPOCHS_LIST = list(range(100, 500, 100))  # [100,200,300,400,500,600]
    ...
    # 默认配置（适配优化后的EEGConformer）
    config.setdefault('batch_size', 16)  # 减小batch_size适配小样本
    config.setdefault('lr', 0.0001)  # 降低学习率避免震荡
    config.setdefault('random_seed', 0)
    config.setdefault('weight_decay', 1e-5)  # 增大权重衰减防止过拟合
    config.setdefault('label_smoothing', 0.01)  # 轻量标签平滑
    config.setdefault('timestamp', time.strftime('%Y-%m-%d--%H-%M', time.localtime()))
    config.setdefault('network', 'EEGConformer')
    config.setdefault('device', 'cuda:0')
    config.setdefault('network_args', {})

    # EEGConformer核心参数（优化后）
    config['network_args'].setdefault('emb_size', 40)
    config['network_args'].setdefault('depth', 4)  # 减少深度降低过拟合
    config['network_args'].setdefault('n_classes', 10)
    config['network_args'].setdefault('num_heads', 8)  # 8头更稳定
    config['network_args'].setdefault('drop_p', 0.3)  # 降低dropout
    ...
    for sub_id, train_datafile in subject_list:
        ...
        for train_epochs in EPOCHS_LIST:
            print(f"\n=====================================")
            print(f"开始训练 EEGConformer ：【{sub_id}】 | Epoch数：{train_epochs}")
            print(f"=====================================")
