...
# ===================== 工具函数 =====================
def setRandom(seed):
    import random  # 补充random导入（避免未定义）
    random.seed(seed)
    np.random.seed(seed)
...
def load_model_weights(model, weight_path, device):
    """
    安全加载TransNet模型权重，适配参数不匹配问题
    """
    # 加载权重文件
    state_dict = torch.load(weight_path, map_location=device)

    # 预热模型（验证前向传播，TransNet无动态层）
    model.eval()
    with torch.no_grad():
        dummy_input = torch.randn(1, 1, 32, 1000).to(device)
        _ = model(dummy_input)  # TransNet仅返回分类输出
    ...
    print(f"✅ 成功加载TransNet权重：{weight_path} | 匹配参数数：{len(filtered_state_dict)}")
    return model


def scan_subjects_models(out_root, network_name='TransNet'):
    """
    扫描所有被试的TransNet训练模型路径
    """
    ...
    for sub_dir in out_root.glob(f"{network_name}/sub*"):
        ...
        for epoch_dir in epoch_dirs:
            ...
            # 可选：同时支持读取final模型（根据训练代码保存的文件名调整）
            # final_model_path = list(epoch_dir.glob("final_*/final_model.pth"))
            # if final_model_path:
            #     epoch_list.append(epoch_num)
            #     model_paths[epoch_num] = final_model_path[0]
            if global_best_path:
                epoch_list.append(epoch_num)
...
if __name__ == '__main__':
    ...
    OUT_ROOT = r'F:/EEG-TransNet-main/output/bci_handwriting_data_Pinyin'
    CONFIG_PATH = r'F:/EEG-TransNet-main/config/Hand_Writing_transnet.yaml'  # 改为TransNet配置文件
    NETWORK_NAME = 'TransNet'  # 模型名称改为TransNet
